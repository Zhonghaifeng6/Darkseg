...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir, dark_dir, scale=1):
    ...
    def __getitem__(self, i):
        ...
        dark = self.preprocess(dark, self.scale)
        mask = self.preprocess(mask, self.scale)

        return {'image': torch.from_numpy(img), 'dark': torch.from_numpy(dark),'mask': torch.from_numpy(mask)}
