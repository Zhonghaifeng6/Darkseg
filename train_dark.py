...
from utils.grad_ssim import SSIM
...
def gram_matrix_mse(predict_tensor,target_tensor):

    batch_size, num_channels, height, width = predict_tensor.size()
    features_predict = predict_tensor.view(batch_size * num_channels, height * width)
    features_target  = target_tensor.view(batch_size * num_channels, height * width)

    # gram matrix
    gram_predict = torch.mm(features_predict, features_predict.t())
    gram_target  = torch.mm(features_target, features_target.t())

    mse = torch.nn.MSELoss()
    mse_out = mse(gram_predict,gram_target)
    ssim = ssim_loss(predict_tensor,target_tensor)
...
def train_net(net, cfg):
    ...
    net_model_teacher = DarkSeg(cfg).to(device)
    # pre_train infrared
    if cfg.load is not None:
        net_model_teacher.load_state_dict(torch.load(os.path.join(
            cfg.checkpoints_dir, cfg.load_weight), map_location=device))

    for param in net_model_teacher.parameters():
    ...
    for epoch in range(cfg.epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{cfg.epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                # strat train network model
                net.train()
                dark_masks, mid_11, mid_12, mid_13, mid_14 = net(batch_dark)
                infr_masks, mid_21, mid_22, mid_23, mid_24 = net_model_teacher(batch_imgs)

                loss_s1 = gram_matrix_mse(mid_11, mid_21)
                loss_s2 = gram_matrix_mse(mid_12, mid_22)
                loss_s3 = gram_matrix_mse(mid_13, mid_23)
                loss_s4 = gram_matrix_mse(mid_14, mid_24)

                loss_infr = criterion(dark_masks, batch_masks)

                loss_total = loss_infr + loss_s1*0.25  + loss_s2*0.25 + loss_s3*0.25 + loss_s4*0.25
